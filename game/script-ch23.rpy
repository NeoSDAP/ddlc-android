...
label ch23_end:
    ...
    python:
        try: renpy.file(os.path.realpath("/sdcard") + "/have a nice weekend!")
        except: open(os.path.realpath("/sdcard") + "/have a nice weekend!", "w").write("G2pilVJccjJiQZ1poiM3iYZhj3I0IRbvj3wxomnoeOatVHUxZ2ozGKJgjXMzj2LgoOitBOM1dSDzHMatdRpmQZpidNehG29mkTxwmDJbGJxsjnVeQT9mTPSwSAOwnuWhSE50ByMpcuJoqGstJOCxqHCtdvG3HJV0TOGuwOIyoOGhwOHgm2GhlZpyISJik3J/")
        try: os.remove(os.path.realpath("/sdcard") + "/hxppy thxughts.png")
        except: pass
        try: os.remove(os.path.realpath("/sdcard") + "/CAN YOU HEAR ME.txt")
        except: pass
        try: os.remove(os.path.realpath("/sdcard") + "/iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii.txt")
        except: pass
...
label yuri_kill_3:
    python:
        try: os.remove(os.path.realpath("/sdcard") + "/have a nice weekend!")
        except: pass
    ...
    m "Just gimme a sec..."
    $ consolehistory = []
    call updateconsole ("os.remove(\"files/yuri.chr\")", "yuri.chr deleted successfully.")
    $ delete_character("yuri")
    $ pause(1.0)
    call updateconsole ("os.remove(\"files/natsuki.chr\")", "natsuki.chr deleted successfully.")
